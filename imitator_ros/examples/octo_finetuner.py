...
default_config_file = os.path.join(
    os.path.dirname(__file__), "config", "octo_finetune_config.py"
)
...
def main(argv):
    ...
    # dump config from imitator project
    config = get_config_from_project_name(FLAGS.project_name)
    image_primary_key, image_wrist_key = None, None
    for key, obs in config.obs.items():
        if obs.modality == "ImageModality":
            if obs.camera == "primary":
                image_primary_key = key
                FLAGS.config.frame_transform_kwargs.resize_size.primary = tuple(config.obs[
                    image_primary_key
                ].dim[:2])
            elif obs.camera == "wrist":
                image_wrist_key = key
                FLAGS.config.frame_transform_kwargs.resize_size.wrist = tuple(config.obs[
                    image_wrist_key
                ].dim[:2])
    FLAGS.config.dataset_kwargs.image_obs_keys.primary = image_primary_key
    FLAGS.config.dataset_kwargs.image_obs_keys.wrist = image_wrist_key
    FLAGS.config.dataset_kwargs.state_obs_keys = [
        key for key in config.obs.keys() if config.obs[key].modality == "FloatVectorModality"
    ]
    FLAGS.config.dataset_kwargs.absolute_action_mask = [True] * config.actions.dim
